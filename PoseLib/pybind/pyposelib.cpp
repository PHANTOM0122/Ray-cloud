...
std::pair<CameraPose, py::dict> estimate_absolute_pose_wrapper(const std::vector<Eigen::Vector2d> points2D,
    ...
    RansacStats stats = estimate_absolute_pose(points2D, points3D, camera, ransac_opt, bundle_opt, &pose, &inlier_mask, runRefine);
...
std::pair<CameraPose, py::dict> estimate_generalized_relative_pose_wrapper(
    ...
    const std::vector<py::dict> &cameras2_dict, const py::dict &ransac_opt_dict, const py::dict &bundle_opt_dict, const int runRefine) {

    std::cout << "P5+1R new code." << std::endl;
    ...
    RansacStats stats = estimate_generalized_relative_pose(matches, camera1_ext, cameras1, camera2_ext, cameras2,
                                                           ransac_opt, bundle_opt, &pose, &inlier_mask, runRefine);
...
std::pair<CameraPose, py::dict> estimate_p6l_relative_pose_wrapper(
    const std::vector<Eigen::Vector2d> points2D, 
    const std::vector<Eigen::Vector3d> points3D, 
    const std::vector<Eigen::Vector3d> lines3D, 
    ...
    std::cout << "P6L new code." << std::endl;
    ...
    RansacStats stats = estimate_p6l_relative_pose(points2D, points3D, lines3D, cameras1, cameras2, ransac_opt, bundle_opt, &pose, &inlier_mask, runRefine);

    py::dict output_dict;
    write_to_dict(stats, output_dict);
    pose.q = pose.q/pose.q.norm();
    // output_dict["inliers"] = convert_inlier_vectors(inlier_mask);
    return std::make_pair(pose, output_dict);
}
...
PYBIND11_MODULE(poselib, m) {
    ...
    m.def("estimate_absolute_pose", &poselib::estimate_absolute_pose_wrapper, py::arg("points2D"), py::arg("points3D"),
          py::arg("camera_dict"), py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(),py::arg("runRefine"),
    ...
    m.def("estimate_generalized_relative_pose", &poselib::estimate_generalized_relative_pose_wrapper,
          ...
          py::arg("camera2_dict"), py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(), py::arg("runRefine"),
          "Generalized relative pose estimation with non-linear refinement. Note: This is modified implemenation for ray cloud.");
    m.def(
    ...
    m.def("estimate_p6l_relative_pose", &poselib::estimate_p6l_relative_pose_wrapper,
          py::arg("points2D"), py::arg("points3D"), py::arg("lines3D"), py::arg("camera1_dict"), 
          py::arg("camera2_dict"), py::arg("ransac_opt") = py::dict(), py::arg("bundle_opt") = py::dict(), py::arg("runRefine"),
          "p6l relative pose estimation with non-linear refinement.");
