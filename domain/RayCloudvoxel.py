...
class Raycloudvoxel(Master):
    def __init__(self, dataset_path, output_path):
        ...
        self.map_type = "RayCloudVoxel" # Ray Clouds - DBSCAN added
        ...
        self.datascale = variable.getScale(self.dataset)
        print('Voxel size (m):', variable.VOXEL_SIZE)
        self.voxel_size = variable.VOXEL_SIZE / self.datascale

        # 3D points COLMAP IDs
    ...
    def makeLineCloud(self):
        ...
        print("Ray Cloud Voxel: Line between points and center")
        ...
        print('self.center_pts:', self.center_pts)

        self.points_3D, self.line_3d, self.ind_to_id, self.id_to_ind, self.center_points, self.lines_center_indices = line.drawlines_ray_clouds_voxelize(_pts_3d, _pts_ids, self.center_pts, self.voxel_size)
        ...
        if visualize == True:
            ...
            pts0_0 = pts[np.where(self.lines_center_indices==0)[0]] - lines[np.where(self.lines_center_indices==0)[0]] * 10
            pts0_1 = pts[np.where(self.lines_center_indices==0)[0]] + lines[np.where(self.lines_center_indices==0)[0]] * 10
            pts1_0 = pts[np.where(self.lines_center_indices==1)[0]] - lines[np.where(self.lines_center_indices==1)[0]] * 10
            pts1_1 = pts[np.where(self.lines_center_indices==1)[0]] + lines[np.where(self.lines_center_indices==1)[0]] * 10

            pair0 = []
            pair1 = []
            num_pts0 = len(pts0_0)
            num_pts1 = len(pts1_0)
            portion1 = int(num_pts1 * 0.01)

            line_indices0 = np.random.choice(np.arange(num_pts0), portion1)
            line_indices1 = np.random.choice(np.arange(num_pts1), portion1)

            for i in line_indices0:
                pair0.append((i, i))
            for i in line_indices1:
                pair1.append((i, i))
            ...
            pcd0_0 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts0_0))
            pcd0_1 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts0_1))
            pcd1_0 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts1_0))
            pcd1_1 = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts1_1))

            lcd = o3d.geometry.LineSet.create_from_point_cloud_correspondences(pcd_0, pcd_1, pair)
            lcd0 = o3d.geometry.LineSet.create_from_point_cloud_correspondences(pcd0_0, pcd0_1, pair0)
            lcd1 = o3d.geometry.LineSet.create_from_point_cloud_correspondences(pcd1_0, pcd1_1, pair1)

            pcd_origin = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pts))
            pcd_origin_0 = o3d.geometry.PointCloud(
                o3d.utility.Vector3dVector(pts[np.where(self.lines_center_indices == 0)[0]]))
            pcd_origin_1 = o3d.geometry.PointCloud(
                o3d.utility.Vector3dVector(pts[np.where(self.lines_center_indices == 1)[0]]))

            pcd_origin.paint_uniform_color([0, 0, 0])
            pcd_origin_0.paint_uniform_color([0, 0, 0])
            pcd_origin_1.paint_uniform_color([0, 0, 0])
            # o3d.visualization.draw_geometries([pcd_origin_0, pcd_origin_1])

            anchor0_color = np.array([155/255, 34/255, 38/255], dtype=np.float64)
            anchor1_color = np.array([0/255, 119/255, 182/255], dtype=np.float64)
            pcd_origin_0.paint_uniform_color(anchor0_color)
            pcd_origin_1.paint_uniform_color(anchor1_color)

            print('len pts[np.where(self.lines_center_indices == 0)[0]]):', len(pts[np.where(self.lines_center_indices == 0)[0]]))
            print('len pts[np.where(self.lines_center_indices == 1)[0]]):', len(pts[np.where(self.lines_center_indices == 1)[0]]))

            lcd.paint_uniform_color([0.7, 0.7, 0.7])
            lcd0.paint_uniform_color(anchor0_color)
            lcd1.paint_uniform_color(anchor1_color)
            o3d.visualization.draw_geometries([pcd_origin_0, pcd_origin_1])
            o3d.visualization.draw_geometries([pcd_origin_0, pcd_origin_1, lcd0, lcd1])
    ...
    def estimatePose(self, query_id):
        ...
        if self._x1.shape[0] >= 6:
            ...
            cam_virtual = {'model':'SIMPLE_PINHOLE', 'width':1,'height':1, 'params':[1,0,0]}
    ...
    def recoverPts(self, estimator, sparsity_level, noise_level):
        ...
        print("Voxel-based ray cloud recover 3D points", "\n")
        if estimator == 'SPF':
            ...
            if variable.LINE_REJECTION: # With line-rejection
                ...
                info = [sparsity_level, noise_level, 0.0, estimator, self.num_clusters, f"{str(variable.VOXEL_SIZE)}mvoxel_{str(variable.REJECTION_RATIO)}rejectcoarse"]
